export default function CardJSON() {
    return JSON.parse(JSON.stringify([
        {
            img_src: 'https://cdn.discordapp.com/attachments/913641847086252053/954239751949516810/unknown.png',
            text: 'GCI OpenMRS (2019).',
            source: 'https://github.com/aayushGarg-1748/open-mrs-mail',
            label: 'HTML CSS',
            deploy: 'https://aayushgarg-1748.github.io/open-mrs-mail/'
        },
        {
            img_src: 'https://cdn.discordapp.com/attachments/913641847086252053/954237901934305280/unknown.png',
            text: 'GCI OpenMRS 404 Page Task (2019).',
            source: 'https://github.com/aayushGarg-1748/openmrs-404',
            label: 'HTML CSS',
            deploy: 'https://aayushgarg-1748.github.io/openmrs-404/'
        },
        {
            img_src: "https://upload.wikimedia.org/wikipedia/commons/thumb/0/03/Simple_Periodic_Table_Chart-blocks.svg/1200px-Simple_Periodic_Table_Chart-blocks.svg.png",
            text: 'MCQ Periodic Table Quiz, Randomly generated 10 questions.',
            source: 'https://github.com/gamer-1478/quiz-app',
            label: 'Chemistry-Quiz',
            deploy: 'https://quiz-app-aayush.netlify.app/'
        },
        {
            img_src: "https://socialify.git.ci/gamer-1478/cpu-benchmark/image?description=1&font=Raleway&issues=1&language=1&pattern=Charlie%20Brown&pulls=1&stargazers=1&theme=Dark",
            text: 'Cpu-Benchmark',
            label: 'Python-Project',
            source: 'https://github.com/gamer-1478/cpu-benchmark/blob/main/bench.py'
        },
        {
            img_src: "https://www.researchgate.net/publication/327714444/figure/fig3/AS:672028931653638@1537235591748/Robotic-platform-used-in-this-study-a-Original-design-of-the-InMoov-robot-and-b.ppm",
            text: 'Cybortics-Humanoid',
            label: 'Robotics Project',
            path: '/cybortics-humanoid'
        },
        {
            img_src: "https://cdn.discordapp.com/attachments/913641847086252053/954238103525146644/unknown.png",
            text: 'Online Simulated A.I. For A Simulated Home',
            label: 'AI Project',
            deploy: '/ai-dsm',
            source: 'https://github.com/gamer-1478/ai-dsm'

        },
        {
            img_src: 'https://socialify.git.ci/gamer-1478/TaskPlannerProject/image?description=1&font=Raleway&issues=1&language=1&pattern=Charlie%20Brown&pulls=1&stargazers=1&theme=Dark',
            text: 'Task Planner App For ATL Tinkerpreneur',
            source: 'https://github.com/gamer-1478/TaskPlannerProject',
            label: 'Android Studio'
        },
        {
            img_src: 'https://socialify.git.ci/gamer-1478/toycathon/image?description=1&font=Raleway&issues=1&language=1&pattern=Charlie%20Brown&pulls=1&stargazers=1&theme=Dark',
            text: 'Toycathon is a GOV hosted Comp, for which we made a android app with a physical board game and won.',
            source: 'https://github.com/gamer-1478/toycathon',
            label: 'Android Game Flutter'
        },
        {
            img_src: 'https://socialify.git.ci/gamer-1478/website-ssr/image?description=1&font=Raleway&issues=1&language=1&pattern=Charlie%20Brown&pulls=1&stargazers=1&theme=Dark',
            text: 'Server Side Rendering With React Deployed On Heroku',
            source: 'https://github.com/gamer-1478/website-ssr',
            label: 'React SSR Express JS'
        },
        {
            img_src: 'https://socialify.git.ci/gamer-1478/chepobin-public/image?description=1&font=Raleway&issues=1&language=1&pattern=Charlie%20Brown&pulls=1&stargazers=1&theme=Dark',
            text: 'Chepobin is a PasteBin Type OpenSource Text Sharing Hub.',
            source: 'https://github.com/gamer-1478/chepobin-public',
            label: 'Express JS Chepobin',
        }, {
            img_src: 'https://socialify.git.ci/gamer-1478/syntax-prelims/image?description=1&font=Raleway&issues=1&language=1&pattern=Charlie%20Brown&pulls=1&stargazers=1&theme=Dark',
            source: 'https://github.com/gamer-1478/syntax-prelims',
            text: 'Syntax Event Entry For TS, A simple and robust music streaming platform.',
            label: 'NodeJS-Project'
        }, {
            img_src: 'https://cdn.discordapp.com/attachments/913641847086252053/954242877574221844/unknown.png',
            source: 'https://github.com/ion05/alphabit-prelims',
            text: 'Alphabit Event Entry For TS',
            label: 'NodeJS-Project'
        }, {
            img_src: 'https://socialify.git.ci/gamer-1478/spotify-listen-along/image?description=1&font=Raleway&issues=1&language=1&pattern=Charlie%20Brown&pulls=1&stargazers=1&theme=Dark',
            source: 'https://github.com/gamer-1478/spotify-listen-along',
            text: 'Free Spotify Listen Along',
            label: 'NodeJS-Project'
        }, {
            text: 'A raylib game of snake in c++.',
            img_src: 'https://socialify.git.ci/gamer-1478/snake-raylib/image?description=1&font=Raleway&issues=1&language=1&pattern=Charlie%20Brown&pulls=1&stargazers=1&theme=Dark',
            source: 'https://github.com/gamer-1478/snake-raylib',
            label: 'Raylib'
        }, {
            text: 'Exun Hardware docker first try.',
            img_src: 'https://socialify.git.ci/gamer-1478/exun_hardware/image?description=1&font=Raleway&issues=1&language=1&pattern=Charlie%20Brown&pulls=1&stargazers=1&theme=Dark',
            source: 'https://github.com/gamer-1478/exun_hardware',
            label: 'Docker'
        }, {
            text: 'Programming Platform Like Hackkerank Built From Ground Up For Inhoue Events.',
            source: 'https://github.com/techsyndicate/Programming-Platform',
            img_src: 'https://cdn.discordapp.com/attachments/913641847086252053/954235822264172584/unknown.png',
            label: 'Programming-Platform',
            deploy:'https://ts-prog.herokuapp.com'
        }, {
            text: 'Website Developed For Anirudh Chaturvedi.',
            source: 'https://github.com/gamer-1478/portfolio-site',
            img_src: 'https://cdn.discordapp.com/attachments/913641847086252053/954235238547075165/unknown.png',
            label: 'Anirudh-Portfolio',
            deploy: 'https://www.anirudhchaturvedi.me/'
        }
    ]))
}
